...
exports.getPlayableServer = exports.generateIframeUrl = exports.getEpisodeServersByQuality = exports.getEpisodeServers = exports.getAnimeEpisodes = void 0;
...
const getAnimeEpisodes = async (animeId) => {
    try {
        ...
        const $$ = (0, cheerio_1.load)(noscript);
        const episodes = [];
        $$(".divv11").each((episodeIndex, episodeElement) => {
            ...
            $$(episodeElement).find("li").each((serverIndex, serverElement) => {
                ...
                servers.push({
                    ...
                    url: (0, exports.generateIframeUrl)({
                        name: $server.text().trim(),
                        type: $server.attr("type") || "",
                        quality: $server.attr("quality-data") || "",
                        data: $server.attr("data") || "",
...
const getEpisodeServers = async (animeId, episodeNumber) => {
    const data = await (0, exports.getAnimeEpisodes)(animeId);
...
const getEpisodeServersByQuality = async (animeId, episodeNumber, quality) => {
    const servers = await (0, exports.getEpisodeServers)(animeId, episodeNumber);
    return servers.filter(server => server.quality === quality);
};
exports.getEpisodeServersByQuality = getEpisodeServersByQuality;
const generateIframeUrl = (server) => {
...
exports.generateIframeUrl = generateIframeUrl;
const getPlayableServer = async (animeId, episodeNumber, preferredQuality) => {
    let servers = await (0, exports.getEpisodeServers)(animeId, episodeNumber);
    if (preferredQuality) {
    ...
    for (const server of servers) {
        const iframeUrl = (0, exports.generateIframeUrl)(server);
        if (iframeUrl) {
...
exports.getPlayableServer = getPlayableServer;
